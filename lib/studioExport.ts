...
export function buildStudioRepositoryFiles(site: StudioSite, domain?: string, fallbackHost?: string): StudioRepositoryFile[] {
  ...
  const pages = site.pageMode === "multi" ? pageDefinitions : [pageDefinitions[0]];
  const files: StudioRepositoryFile[] = pages.map((page) => ({ path: page.path ? `${page.path}/index.html` : "index.html", content: renderPage(site, page, origin) }));
  files.push({ path: "styles.css", content: stylesheet });
  files.push({ path: "site-data.json", content: JSON.stringify(site, null, 2) });
