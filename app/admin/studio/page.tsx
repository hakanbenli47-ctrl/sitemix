...
type AdminData = { projects: Project[]; leads: Lead[]; subscriptions: Subscription[]; payments: Payment[]; domains: Domain[]; forms: FormEntry[]; users: UserItem[]; audits: Audit[]; sectors: Sector[]; settings: Setting[]; setupRequired?: boolean };
type AdminTab = "overview" | "sites" | "users" | "leads" | "payments" | "domains" | "messages" | "settings" | "logs";

const initialData: AdminData = { projects: [], leads: [], subscriptions: [], payments: [], domains: [], forms: [], users: [], audits: [], sectors: [], settings: [] };
...
function label(value?: string | null) {
  const labels: Record<string, string> = {
    ...
    monthly: "Aylık", yearly: "Yıllık", managed: "SiteMix yönetsin", yearly_setup: "Yıllık kurulum", managed_service: "Yönetim talebi", custom_sector: "Özel sektör",
  };
    ...
    <main className="min-h-screen bg-[#f4f6fb] text-slate-950">
      <div className="grid min-h-screen lg:grid-cols-[250px_1fr]">
        ...
        <section className="min-w-0 p-4 sm:p-6 lg:p-9">
          <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-indigo-600">SiteMix Studio</p><h1 className="mt-2 text-3xl font-black tracking-[-0.05em] sm:text-4xl">{({ overview: "Bütün sistem", sites: "Kurulan siteler", users: "Kullanıcılar", leads: "Satış ve hizmet talepleri", payments: "Paket ve ödemeler", domains: "Domain yönetimi", messages: "Müşteri mesajları", settings: "Sektörler ve sistem ayarları", logs: "İşlem geçmişi" } as Record<AdminTab, string>)[tab]}</h1><p className="mt-2 text-sm font-semibold text-slate-500">SiteMix’e giren, çıkan ve değişen bütün kayıtlar tek yerde.</p></div><button onClick={load} className="min-h-11 rounded-full bg-white px-5 text-xs font-black shadow-sm">Verileri yenile</button></header>
