    ...
    >
      <header className="sticky top-0 z-40 border-b backdrop-blur-2xl" style={{ borderColor: `${theme.foreground}12`, background: `${theme.background}E8` }}>
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3.5 sm:px-7">
          <button type="button" onClick={() => navigate("home")} className="flex min-w-0 items-center gap-3 text-left">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-cover bg-center text-sm font-black shadow-lg" style={{ backgroundColor: theme.accent, backgroundImage: site.media?.logo ? `url("${site.media.logo}")` : undefined, color: theme.background }}>{site.media?.logo ? <span className="sr-only">{site.businessName} logosu</span> : site.businessName.charAt(0) || "S"}</span>
      ...
      <main key={activePage} className="animate-[site-page-in_.38s_ease-out]">
        ...
        {visibleSections.map((section) => {
              ...
              <section id="site-section-hero" key={section.id} className={`relative overflow-hidden px-5 ${design.heroStyle === "immersive" ? "py-20 sm:py-32" : sectionSpacing} sm:px-9`}>
                ...
                <div className={`relative mx-auto grid max-w-6xl items-center gap-12 ${design.heroAlign === "center" ? "text-center" : "lg:grid-cols-[1.08fr_.92fr]"}`}>
                  <div className={design.heroAlign === "center" ? "mx-auto max-w-4xl" : ""}>
                    <div className="inline-flex items-center gap-2 rounded-full border px-3.5 py-2 text-[9px] font-black uppercase tracking-[.16em]" style={{ borderColor: `${theme.accent}35`, background: `${theme.accent}0D`, color: theme.accent }}><span className="h-1.5 w-1.5 rounded-full" style={{ background: theme.accent }} />{section.eyebrow || `${site.location} · ${site.sector}`}</div>
                    ...
                    <div className={`mt-8 flex flex-wrap gap-3 ${design.heroAlign === "center" ? "justify-center" : ""}`}>
                      <button type="button" onClick={contactAction} className="rounded-full px-6 py-3.5 text-xs font-black shadow-xl transition hover:-translate-y-0.5" style={{ background: theme.accent, color: theme.background }}>{section.ctaLabel || "Hemen bilgi al"} <span className="ml-2">↗</span></button>
                  ...
                  <div className={`relative mx-auto w-full max-w-xl ${design.heroAlign === "center" ? "mt-2" : ""}`}>
                    {site.media?.hero ? <div className={`${cardRadius} relative min-h-[430px] overflow-hidden border bg-cover bg-center shadow-[0_35px_90px_rgba(0,0,0,.2)]`} style={{ borderColor: `${theme.foreground}12`, backgroundImage: `linear-gradient(to top, rgba(5,6,8,.82), rgba(5,6,8,.05) 65%), url("${site.media.hero}")`, color: "white" }}><div className="absolute inset-x-0 bottom-0 p-6 sm:p-8"><p className="text-[9px] font-black uppercase tracking-[.16em] opacity-55">{site.businessName}</p><h3 className="mt-3 max-w-md text-3xl font-black leading-[1] tracking-[-.05em]">Gerçek işiniz, gerçek görüntünüz.</h3><button type="button" onClick={contactAction} className="mt-5 rounded-full px-5 py-3 text-[10px] font-black" style={{ background: theme.accent, color: theme.background }}>{section.ctaLabel || "Görüşelim"}</button></div></div> : <div className={`${cardRadius} overflow-hidden border shadow-[0_35px_90px_rgba(0,0,0,.14)]`} style={{ borderColor: `${theme.foreground}12`, background: design.heroStyle === "immersive" ? theme.foreground : `${theme.accent}09`, color: design.heroStyle === "immersive" ? theme.background : theme.foreground }}>
          ...
          if (section.type === "faq") {
            return <section id="site-section-faq" key={section.id} className={`border-t px-5 sm:px-9 ${sectionSpacing}`} style={{ borderColor: `${theme.foreground}12` }}><div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.7fr_1.3fr]"><div><p className="text-[9px] font-black uppercase tracking-[.18em]" style={{ color: theme.accent }}>{section.eyebrow || "Merak edilenler"}</p><h2 className="mt-4 text-4xl font-black leading-[1] tracking-[-.055em]">{section.title}</h2><p className="mt-4 text-xs font-medium leading-6 opacity-48">{section.text}</p></div><div className="divide-y border-y" style={{ borderColor: `${theme.foreground}12` }}>{(section.items || []).map((item, index) => <details key={`${item}-${index}`} className="group py-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-black"><span>{item}</span><span className="grid h-7 w-7 place-items-center rounded-full transition group-open:rotate-45" style={{ background: `${theme.accent}12`, color: theme.accent }}>+</span></summary><p className="mt-3 pr-10 text-xs font-medium leading-6 opacity-55">{section.answers?.[index] || "Bu konuda ihtiyacınıza göre net bilgi paylaşmak için bizimle iletişime geçebilirsiniz."}</p></details>)}</div></div></section>;
          }
